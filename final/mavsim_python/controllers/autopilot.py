...
from controllers.pid_selftune import selftuningPID


class Autopilot:
    def __init__(self, ts_control):
        # instantiate lateral-directional controllers
        self.roll_from_aileron = selftuningPID(
                    kp=0.5,
                    ki = 0.05,
                    kd=0.1,
                    limit=np.radians(45),
                    learning_rate=0.001)
        self.course_from_roll = selftuningPID(
                        kp=0.5,
                        ki = 0.05,
                        kd=0.1,
                        limit=np.radians(30),
        ...
        self.pitch_from_elevator = selftuningPID(
                        kp=0.5,
                        ki = 0.05,
                        kd=0.1,
                        limit=np.radians(45),
                        learning_rate=0.001)
        self.altitude_from_pitch = selftuningPID(
                        kp=0.5,
                        ki = 0.05,
                        kd=0.1,
                        limit=np.radians(45),
                        learning_rate=0.001)
        self.airspeed_from_throttle = selftuningPID(
                        kp=0.5,
                        ki = 0.05,
                        kd=0.1,
                        limit=np.radians(45),
